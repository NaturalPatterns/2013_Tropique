...
def display_player():
	...
	for player in my_players:
		...
		if (player[4] >= 25) :
			...
			#print "my player x , my ghost = ", player[0],my_ghost[a][0]
			#cv2.putText(imaplay,str(my_dist), (player[0]+10,player[1] ), 1 , 1, (255,255,255), 1 , 8)
			#cv2.putText(imaplay,str(player[0]), (player[0]+10,player[1]+10 ), 1 , 1, (255,255,255), 1 , 8)
...
#			#msg.setAddress("/"+str(the_player))
#			msg.setAddress("/len")
#			msg.append(float(float(player[8])/1000))
#			msg.append(float(float(player[10])/1000))
#			send_osc(msg)
...
#			msg.append(1)
#			send_osc(msg)
#			if ( (player[3] > 50) and ((player[0] < 131) or (player[0] > 600)) ):
#				msg = OSC.OSCMessage()
#				#msg.setAddress("/"+str(the_player))
#				msg.setAddress("/presence")
#				msg.append(the_player)
...
def calc_angle(each):
	...
	try:
		if int(each[0]) == 0:
			angle = ( math.asin(coord_x/coord_z))
			x_pos = 100
		if int(each[0]) == 1:
			angle = ( math.asin(coord_x/coord_z))- 1.2
			x_pos = 300
		if int(each[0]) == 2:
			angle = ( math.asin(coord_x/coord_z))
			x_pos = 300
		...
		x= x_pos+int ((math.sin (angle) * coord_z) * coef)
		z= 50 + int((math.cos (angle) * coord_z) *coef)
		coord_y -= z /5
		#y = each[3]
		y = coord_y
		#print "x2z2  angle=", x, z, angle
		test_players(x,z*2, y)
...
while 1:
	...
	try :	
		...
		display_player()
		"""
		for y in range (5-nbr_player):
			for x in range(5):
				my_players[y+nbr_player][x]=0 
		"""
