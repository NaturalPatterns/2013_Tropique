...
def show_depth():
	...
	try :	
		...
		nbr_in_group=0
		my_group = [ [ 0 for i in range(15) ] for j in range(15) ]
		...
		for nbr in range (nbr_player ):
			...
			if ((my_group[nbr][0]!=0) and (deja_fait[nbr]==0)):
				...
				for nbr2 in range (1, my_group[nbr][0] ) :	
					print "la je suis au ",nbr2, "qui contien", my_group[nbr2][0]
					for nbr3 in range(my_group[nbr2][0]):
						print "la je range le ", nbr3 ," du ",my_group[(my_group[nbr][nbr2+1])] [nbr3+1], "dans le ", (my_group[nbr2][0])+(my_group[nbr][0]), "du ", nbr

						my_group[nbr][(my_group[nbr2][0])+(my_group[nbr][0])] = my_group[(my_group[nbr][nbr3+1])] [nbr3+1]
					if (my_group [my_group[nbr][nbr2+1]][0]) == 1:
						deja_fait[nbr2]=1
						my_group[my_group[nbr][nbr2+1]]= [0 for i in range(15)] 			
