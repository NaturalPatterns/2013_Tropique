...
global my_trigo
my_trigo = [ [ 0 for i in range(3) ] for j in range(10) ]
...
def show_depth():
	...
	for desin in range(nbr_player):
		cv.Circle(image2,(int (my_mat[desin][3]) ,int (my_mat[desin][1]) ),10,(my_var,128,0),1)
		font = cv.InitFont(cv.CV_FONT_HERSHEY_SIMPLEX, 1, 1, 0, 3, 8) #Creates a font
		cv.PutText(image2,str(desin), (int (my_mat[desin][3]) ,int (my_mat[desin][1])),font, my_var)
	"""
	...
	try :	
		...
		nbr_player=0
		print"data =" ,Donnee 
		Donnee = Donnee [:((len(Donnee))- 1)]
		print"data =" ,Donnee 
		#Donnee = ((angle + x + y + "o")*nbr_player)+";"
		datasplit = Donnee.split("O")
		print "datasplit =" , datasplit
		datasplit1 = datasplit[0].split(" o ")
		#print (datasplit1[0])
		my_tour =0 
		if Donnee!="":
			for dat in Donnee.split("O"):
				print "my dat =" ,dat
				for datas in dat.split("o"):
					print datas
					nbr =0

					for my_dat in datas.split(" "):
						if my_dat!="": 
							print "my-dat =",my_dat
							try :
								test_var = int (my_dat)
							except:
								print "badnews on = ", my_dat
								if my_dat!="":
									nbr_player = nbr_player - 1
									if my_dat=='192.168.1.163':
										print "jai trouvé la ", my_dat
									print 'erreur imposssssssssssssssssible'
							else :
								my_mat[nbr_player][nbr] = int (my_dat)
								print "ok for =",my_dat
								if nbr ==0 :
									my_mat[nbr_player][0]= my_tour
								nbr = nbr + 1
					#my_mat[nbr_player][0]= my_tour
					nbr_player = nbr_player +1
		...
		print "numbre of player = ", nbr_player


		for trigo in range(nbr_player): 
			if my_mat[trigo][0] == 0 :
				my_mat[trigo][3] = my_mat[trigo][3] - 600
				my_trigo[trigo][1] = float((float(my_mat[trigo][3]))/1000)
				my_trigo[trigo][0] = float(float(my_mat[trigo][1]) / 640)
				print "mytrigo = ", my_trigo[trigo][0] ,  my_trigo[trigo][1] 
				my_trigo[trigo][2] = math.atan(my_trigo[trigo][0]/my_trigo[trigo][1])

				mon_angle = math.atan(my_trigo[trigo][0]/my_trigo[trigo][1])
				mon_hypo = math.sqrt( (my_trigo[trigo][0]*my_trigo[trigo][0]) + (my_trigo[trigo][1]*my_trigo[trigo][1]) )
				my_trigo[trigo][0]  = mon_hypo * math.sin(( ( float(seuil_detect) /100) + mon_angle  ))
				my_trigo[trigo][1]  = mon_hypo * math.cos(( ( float(seuil_detect) /100) + mon_angle  ))

				print "mon angle = ",(( ( float(seuil_detect) /100) + mon_angle  )) , my_trigo[trigo][0] , my_trigo[trigo][1]
				print "mytrigo = ", my_trigo[trigo][2] 
				my_mat[trigo][1] = int(my_mat[trigo][1] /2)
	...
	for desin in range(nbr_player):
		cv.Circle(image2,(int (my_trigo[desin][0]*300) ,int (my_trigo[desin][1]*500) ),10,(my_var,128,0),1)
		font = cv.InitFont(cv.CV_FONT_HERSHEY_SIMPLEX, 1, 1, 0, 3, 8) #Creates a font
		cv.PutText(image2,str(desin), (int (my_trigo[desin][0]*300) ,int (my_trigo[desin][1]*500)),font, my_var)		
