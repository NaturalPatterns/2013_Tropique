...
class Scenario:
    def __init__(self, N, scenario, volume, VPs, p):
        ...
        self.croix = np.array([d_x*0., d_y/2, VPs[0]['z']], dtype='f') # definition de la position de la croix
        self.roger = np.array([d_x/3., d_y/2, 1.68], dtype='f') #  fixation dot  (AKA Roger?)
...
#                    rotation_ = np.sin(cap_SC-cap_AB)[np.newaxis, :]*rae2xyz(np.array([1., rae_AB[1]+np.pi/2, rae_AB[2]+np.pi/2]))[:, np.newaxis]
                ...
                force[0:3, :] += G_rot / self.nvps * rotation
                force[3:6, :] -= G_rot / self.nvps * rotation
